...
def _main(cfg: DictConfig, output_file):
    ...
    for sample in progress:
        ...
        first_p=cfg.generation.first_p
        last_p=cfg.generation.last_p
        kmax=cfg.generation.kmax
        kmin=cfg.generation.kmin
        gen_timer.start()
        ...
        # rws.extend([" ".join([str(item) for item in rw]) for rw in finalized_rws])
    ...
    with open(cfg.common_eval.results_file, "a") as f:
        f.write(",".join(map(str,[cfg.generation.first_p, cfg.generation.last_p, cfg.generation.kmin, cfg.generation.kmax, cw, ap, dal, al, scorer.score(order=4)]))) 
