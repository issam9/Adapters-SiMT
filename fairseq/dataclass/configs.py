...
class GenerationConfig(FairseqDataclass):
    ...
    test_wait_k: int = field(
        default=3,
        metadata={"help": "Lagging during inference."},
    )
    first_p: float = field(
        default=0.5,
        metadata={"help": "Treshold probability for the first step"},
    )
    last_p: float = field(
        default=0.,
        metadata={"help": "Treshold probability for the last step"},
    )
    kmax: int = field(
        default=10,
        metadata={"help": "Maximum waitk lagging"},
    )
    kmin: int = field(
        default=1,
        metadata={"help": "Minimum waitk lagging"},
    )
...
class CommonEvalConfig(FairseqDataclass):
    ...
    results_file: Optional[str] = field(
        default=None, metadata={"help": "csv file to save results"}
    )
